  ...
  function changePage(mode) {
    ...
    switch (mode) {
    case 'https://assets.netdb.at/netdb/html/home/index.html':
  	window.history.pushState('home', 'Netdb | Home', '/home/');
      initHome();
      break;
    case 'https://assets.netdb.at/netdb/html/news/index.html':
  	window.history.pushState('news', 'Netdb | News', '/news/');
      break;
    case 'https://assets.netdb.at/netdb/html/discordbot/index.html':
  	window.history.pushState('discordbot', 'Netdb | Discord Bot', '/discordbot/');
      break;
    case 'https://assets.netdb.at/netdb/html/docs/index.html':
  	window.history.pushState('docs', 'Netdb | Docs', '/docs/');
      break;
    case 'https://assets.netdb.at/netdb/html/login/index.html':
  	window.history.pushState('login', 'Netdb | Login', '/login/');
    initLogin();
      break;
    case 'https://assets.netdb.at/netdb/html/dashboard/index.html':
    window.history.pushState('dashboard', 'Netdb | Dashboard', '/dashboard/');
     initDashboard();
      break;
      case 'https://assets.netdb.at/netdb/html/profile/index.html':
      displayNavandFooter();
  ...
  window.onpopstate = function() {
    changePage('https://assets.netdb.at/netdb/html/' + window.location.toString().split('/')[3] + '/index.html');
  };
  ...
  var validation = Array.prototype.filter.call(forms, function(form) {
    form.addEventListener('submit', function(event) {
      if (form.checkValidity() === false) {
        ...
        request.open('POST', 'https://api.netdb.at/auth/pw', true);
        request.onload = () => {

          if(!request.response.includes('error')) {
            document.cookie = 'token=' + request.response + ';path=/;Domain=netdb.at;SameSite=Strict; Secure';
            login();
            changePage('https://assets.netdb.at/html/home/index.html');
          }
          else
          {
...
function displayNavandFooter() {
  ...
  function initProfile() {
    if(!data) {
     changePage('https://assets.netdb.at/html/login/index.html');
     return;
    }
  ...
  function initDashboard() {
    if(!data) {
     changePage('https://assets.netdb.at/netdb/html/login/index.html');
     return;
    }
...
function switchLoginForm() {
  ...
  function loadApps() {
      ...
      req.open('GET', 'https://api.netdb.at/app/get');
  ...
  function initHome() {
    ...
    request.open('GET', 'https://api.netdb.at/info', true);
    ...
    inputBox.onkeyup = (e)=>{
        ...
        if(userData != "") {
          sendGetAsyncRequest("https://api.netdb.at/search/?type=simple&query=" + userData);
        }else{
  ...
  const req = new XMLHttpRequest();
  req.responseType = 'json';
  req.open('GET', 'https://api.netdb.at/login');
  ...
  function login() {
    ...
    var oReq = new XMLHttpRequest();
    oReq.open("GET", 'https://api.netdb.at/login', false);
  ...
  function logout() {
    ...
    document.cookie = "token=; expires=Wed, 29 Sept 2004 00:00:00 UTC;Domain=.netdb.at; path=/;SameSite=Strict; Secure";

    changePage('https://assets.netdb.at/netdb/html/home/index.html');

    var request = new XMLHttpRequest();
    request.open('GET', 'https://api.netdb.at/login/revoke', true);
...
function deleteApp(id) {
  var url = "https://api.netdb.at/app/delete?clientId=" + id;
  var request = new XMLHttpRequest();
...
myModalEl.querySelector('form').classList.remove('was-validated');
  ...
  function updateApp(form) {
    ...
    var url = "https://api.netdb.at/app/update";
    var request = new XMLHttpRequest();
  ...
  function createApp(form) {
    var url = "https://api.netdb.at/app/create";
    var request = new XMLHttpRequest();
    request.open('POST', url, true);
    request.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
    request.setRequestHeader('token', data.Token);
  ...
  function SearchMovie(){
   ...
   changePage('https://assets.netdb.at/netdb/html/home/searchresults.html');

   var request = new XMLHttpRequest();
   request.responseType = 'json';
   request.open('GET', 'https://api.netdb.at/search/?type=simple&query=' + query, true);
   request.onload = function() {
   ...
   for(var i = 0;i < 20;i++)
   {
     ...
     result.querySelector('#s_cover').src = 'https://assets.netdb.at/content/image-not-found.jpg';
...
function toggleClientSecret() {
  ...
  function resetClientSecret() {
    var request = new XMLHttpRequest();
    request.open('POST', 'https://api.netdb.at/app/reset', true);
    request.setRequestHeader('token', data.Token);
    request.onload = function() {
        loadApps();
  ...
  function deleteAccount() {
    var request = new XMLHttpRequest();
    request.open('GET', 'https://api.netdb.at/user/delete', true);
    request.setRequestHeader('token', data.Token);
    request.onload = function() {
      logout();
    };
...
function updateProfile(form) {
  var request = new XMLHttpRequest();
  request.open('POST', 'https://api.netdb.at/user/update', true);
  request.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
  request.setRequestHeader('token', data.Token);
  request.onload = function() {
    console.log(request.responseText);
...
function linkAccount() {
  ...
  var oReq = new XMLHttpRequest();
  oReq.open("POST", 'https://api.netdb.at/link', false);
  oReq.setRequestHeader('token', data.Token);
  oReq.send('type=' + type + '&token=' + linktoken);

  document.cookie = "linkacc=; expires=Wed, 29 Sept 2004 00:00:00 UTC;Domain=.netdb.at; path=/;SameSite=Strict; Secure";
  document.cookie = "linktype=; expires=Wed, 29 Sept 2004 00:00:00 UTC;Domain=.netdb.at; path=/;SameSite=Strict; Secure";
